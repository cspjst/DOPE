...
dope_fn_t dispatch_table[] = {
    [DOPE_OP_ADD] = dope_op_add,
    [DOPE_OP_SUB] = NULL,
    [DOPE_OP_MUL] = NULL,
    [DOPE_OP_DIV] = NULL,
    [DOPE_OP_STORE] = dope_op_store,
    [DOPE_OP_SQR] = NULL,
    [DOPE_OP_EXP] = NULL,
    [DOPE_OP_LOG] = NULL,
    [DOPE_OP_SIN] = NULL,
    [DOPE_OP_COMPARE] = NULL,
    [DOPE_OP_GOTO] = dope_op_goto,
    [DOPE_OP_LABEL] = dope_op_print_label,
    [DOPE_OP_PRINT] = dope_op_print,
    [DOPE_OP_NEWLN] = dope_op_println,
    [DOPE_OP_INPUT] = NULL,
    [DOPE_OP_LOOP] = NULL,
    [DOPE_OP_END] = NULL,
    [DOPE_OP_STOP] = dope_op_stop,
    [DOPE_OP_START] = dope_op_start
};
...
const dope_float_t dope_load_number(dope_size_t field, dope_program_t* program, dope_vartab_t* vars, dope_vectab_t* vecs) {
...
void dope_store_number(dope_float_t number, dope_size_t field, dope_program_t* program, dope_vartab_t* vars, dope_vectab_t* vecs) {
    const char* name = dope_field_to_str(program, field);
    if(dope_is_valid_var_name(name)) {
...
void dope_op_add(dope_program_t* program, dope_vartab_t* vars, dope_vectab_t* vecs, dope_data_t* data) {
    const dope_float_t a = dope_load_number(1, program, vars, vecs);
    const dope_float_t b = dope_load_number(2, program, vars, vecs);
    dope_store_number(a + b, 3, program, vars, vecs);
    program->ip++;
}
...
void dope_op_store(dope_program_t* program, dope_vartab_t* vars, dope_vectab_t* vecs, dope_data_t* data) {
    const dope_float_t a = dope_load_number(1, program, vars, vecs);
    dope_store_number(a, 2, program, vars, vecs);
    program->ip++;
}
...
void dope_op_print(dope_program_t* program, dope_vartab_t* vars, dope_vectab_t* vecs, dope_data_t* data) {
    printf("%f\t", dope_load_number(1, program, vars, vecs));
    program->ip++;
}
